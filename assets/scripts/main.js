...
if ("serviceWorker" in navigator) {
  window.addEventListener('load', () => {
    let reg = navigator.serviceWorker.register('./sw.js').then((n) => console.log("success"), (e) => console.log("failure"));
  });
}
...
return new Promise(async (resolve, reject) => {
  for (let recipe_url of RECIPE_URLS) {
    try {
      let recjson = await fetch(recipe_url);
      let rec = await recjson.json();
      recipes.push(rec);
    }
    catch(e) {
      console.error(e);
